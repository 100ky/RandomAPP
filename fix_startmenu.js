...
const StartMenu: React.FC<StartMenuProps> = ({ onStartGame }) => {
  ...
  useEffect(() => {
    // Získat ID všech dostupných avatarů
    const availableGames = getAvailableAvatars();
    const availableIds = availableGames.map(game => game.id);
    setAvailableAvatarIds(availableIds);
    
    // Nastavit všechna data avatarů pro zobrazení
    setAllAvatarsData(avatarData);
    
    const savedAvatarId = localStorage.getItem('userAvatarId');
    const savedPlayerName = localStorage.getItem('playerName');
    
    // Nastavit uložený avatar, ale jen pokud je dostupný
    if (savedAvatarId && availableIds.includes(savedAvatarId)) {
      setSelectedAvatarId(savedAvatarId);
    } else if (availableIds.length > 0) {
      // Jinak nastavit první dostupný avatar
      setSelectedAvatarId(availableIds[0]);
    }
  ...
  // Kontrola, zda je avatar dostupný
  const isAvatarAvailable = (avatarId: string): boolean => {
    return availableAvatarIds.includes(avatarId);
  };
  ...
  const handleStartGame = () => {
    ...
    // Kontrola, zda je vybraný avatar dostupný
    if (!isAvatarAvailable(selectedAvatarId)) {
      setError('Tento avatar není momentálně dostupný');
      return;
    }
  ...
  // Zpracování kliknutí na avatara
  const handleAvatarClick = (avatarId: string) => {
    // Pokud není avatar dostupný, zobrazíme zprávu
                ...
                >
                  <div className={styles.avatarWrapper}>
                    <img 
                      src={avatar.imageUrl} 
                      alt={avatar.name} 
                      style={!isAvatarAvailable(avatar.id) ? {filter: 'grayscale(100%)', opacity: 0.7} : {}}
                    />
                    {selectedAvatarId === avatar.id && isAvatarAvailable(avatar.id) && (
                      <div className={styles.avatarDecoration}></div>
                    )}
                  </div>
                  <span>{avatar.name}</span>
                  {!isAvatarAvailable(avatar.id) && (
                    <div className={styles.avatarUnavailable}>Ve vývoji</div>
                  )}
                </div>
              ))}
            </div>
